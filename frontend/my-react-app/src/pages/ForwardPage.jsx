...
function ForwardPage() {
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold mb-4">Forward Email {id}</h2>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <input {...register('receiver')} placeholder="Receiver (e.g., charlie@example.com)" className="p-2 border w-full" required />
        <textarea {...register('body')} placeholder="Body" className="p-2 border w-full h-32" required />
        <button type="submit" className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Forward</button>
      </form>
    </div>
  );
}
